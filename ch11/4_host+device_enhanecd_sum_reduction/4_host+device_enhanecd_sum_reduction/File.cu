...
__global__ void kernel(unsigned* pData, unsigned* pAnswer) {
	...
	for (register unsigned int stride =BLOCKSIZE; stride > 0; stride /= 2) {
		__syncthreads();
		if (tx < stride) {
...
int main(void) {
	...
	cudaMalloc((void**)&pAnswerDev, 1 * BLOCKSIZE * sizeof(unsigned));
	...
	//CUDA
	cudaMemcpy(&answer, pAnswerDev, 1 * sizeof(unsigned), cudaMemcpyDeviceToHost);

	//CUDA:free the memroy
	cudaFree(pDataDev);
	cudaFree(pAnswerDev);
